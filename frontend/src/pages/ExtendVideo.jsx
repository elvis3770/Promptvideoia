...
const ExtendVideo = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!prompt.trim() || files.length === 0) {
            setError("Please upload a base video and enter a prompt");
            return;
        }
        ...
        try {
            ...
            const res = await api.extendVideo(formData);
            if (res.data.ok) {
                const newJob = {
    ...
    return (
        <div>
            <div className="mb-8">
                <h1 className="text-2xl font-bold text-slate-900">Extend Video</h1>
                <p className="text-slate-500">Continue the story by extending your existing videos.</p>
            </div>
