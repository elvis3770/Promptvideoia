...
const ReferenceImages = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!prompt.trim() || files.length === 0) {
            setError("Please upload reference images and enter a prompt");
            return;
        }
        ...
        try {
            ...
            const res = await api.referenceImages(formData);
            if (res.data.ok) {
                const newJob = {
    ...
    return (
        <div>
            <div className="mb-8">
                <h1 className="text-2xl font-bold text-slate-900">Reference Images</h1>
                <p className="text-slate-500">Generate consistent characters and styles using multiple references.</p>
            </div>
